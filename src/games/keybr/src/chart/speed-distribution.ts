...
/** The fixed population distribution shipped by keybr.com. */
export function makeSpeedDistribution(): Distribution {
  return (cached ??= new Distribution(
    bucketize(smooth(smooth(smooth(speed, 5), 5), 5), 150),
  ));
}

function smooth(data: readonly number[], wnd: number): number[] {
  const result = [...data];
...
function bucketize(data: readonly number[], buckets: number): number[] {
  ...
  for (let i = 0; i < buckets; i++) {
    ...
    let sum = 0;
    for (let j = a; j < b; j++) sum += data[j];
    for (let j = a; j < b; j++) result[j] = sum / (b - a);
  }
  return result;
}
